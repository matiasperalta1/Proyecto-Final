using System;
using System.Collections.Generic;

#nullable disable

namespace ProyectoFinalElectricidadSeret.Models
{
    public partial class CondpagoAnt
    {
        public int Codcpgo { get; set; }
        public int? Dias1 { get; set; }
        public int? Dias2 { get; set; }
        public int? Dias3 { get; set; }
        public int? Dias4 { get; set; }
        public int? Dias5 { get; set; }
        public int? Dias6 { get; set; }
        public int? Dias7 { get; set; }
        public int? Dias8 { get; set; }
        public int? Dias10 { get; set; }
        public int? Dias9 { get; set; }
        public string Descri { get; set; }
        public decimal? Dto1 { get; set; }
        public decimal? Dto2 { get; set; }
        public decimal? Dto3 { get; set; }
        public decimal? Rec1 { get; set; }
        public string Codlis { get; set; }
    }
}
